...
def client(transport: FakeTransport) -> PermutiveClient:
    ...
    instance = PermutiveClient(
        "secret",
        transport=transport,
        retry_policy=RetryPolicy(max_attempts=2, initial_delay=0.001, max_delay=0.001, jitter=0),
    )
...
def test_pagination_and_repeated_token_protection() -> None:
    ...
    iterator = client(transport).iter_all("/cohorts", item_decoder=lambda item: item["id"])
...
def test_batch_preserves_input_order_and_collects_failures() -> None:
    ...
    result = execute_batch([3, 2, 1], operation, progress=lambda done, total: progress.append((done, total)))
