...
class TestAlias(unittest.TestCase):
    ...
    def test_alias_to_json(self):
        ...
        alias = Alias(id="test_id", tag="test_tag", priority=1)
        self.assertEqual(
            alias.to_json(), {"id": "test_id", "tag": "test_tag", "priority": 1}
        )
...
class TestIdentity(unittest.TestCase):
    def setUp(self):
        ...
        self.identity = Identity(
            user_id="test_user", aliases=[self.alias1, self.alias2]
        )
    ...
    def test_identify_call(self, mock_post_static):
        ...
        mock_post_static.assert_called_once_with(
            api_key=api_key, url=expected_url, data=expected_payload
        )
